...
/*
 ...
 - 2015/07/02 (1.42) - renamed GetScrollPosY() to GetSceenY(). Necessary to reduce confusion along with other scrolling functions, because positions (e.g. cursor position) are not equivalent to scrolling amount.
...
void ImGui::UpdateMouseWheel()
{
    ...
    if (g.IO.MouseWheel != 0.0f)
    {
        ...
        else if (!g.IO.KeyCtrl && scroll_allowed)
        {
            ...
            SetWindowSceenY(scroll_window, scroll_window->Scroll.y - g.IO.MouseWheel * scroll_amount);
        }
    }
    if (g.IO.MouseWheelH != 0.0f && scroll_allowed && !g.IO.KeyCtrl)
    {
        ...
        SetWindowSceenX(scroll_window, scroll_window->Scroll.x - g.IO.MouseWheelH * scroll_amount);
    }
}
...
void ImGui::SetWindowSceenX(ImGuiWindow* window, float new_scroll_x)
{
...
void ImGui::SetWindowSceenY(ImGuiWindow* window, float new_scroll_y)
{
...
float ImGui::GetSceenX()
{
    return GImGui->CurrentWindow->Scroll.x;
}

float ImGui::GetSceenY()
{
...
void ImGui::SetSceenX(float scroll_x)
{
    ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::SetSceenY(float scroll_y)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget)
    {
        ...
        if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll && g.NavMoveRequest)
        {
            if (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right)
                SetWindowSceenX(window, ImFloor(window->Scroll.x + ((g.NavMoveDir == ImGuiDir_Left) ? -1.0f : +1.0f) * scroll_speed));
            if (g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down)
                SetWindowSceenY(window, ImFloor(window->Scroll.y + ((g.NavMoveDir == ImGuiDir_Up) ? -1.0f : +1.0f) * scroll_speed));
        }

        // *Normal* Manual scroll with NavSceenXXX keys
        // Next movement request will clamp the NavId reference rectangle to the visible area, so navigation will resume within those bounds.
        ImVec2 scroll_dir = GetNavInputAmount2d(ImGuiNavDirSourceFlags_PadLStick, ImGuiInputReadMode_Down, 1.0f/10.0f, 10.0f);
        if (scroll_dir.x != 0.0f && window->ScrollbarX)
        {
            SetWindowSceenX(window, ImFloor(window->Scroll.x + scroll_dir.x * scroll_speed));
            g.NavMoveFromClampedRefRect = true;
        }
        if (scroll_dir.y != 0.0f)
        {
            SetWindowSceenY(window, ImFloor(window->Scroll.y + scroll_dir.y * scroll_speed));
            g.NavMoveFromClampedRefRect = true;
        }
    }
        ...
        if ((page_up_held && !page_down_held) || (page_down_held && !page_up_held))
        {
            if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll)
            {
                ...
                if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true))
                    SetWindowSceenY(window, window->Scroll.y - window->InnerClipRect.GetHeight());
